...
- 法人信息：法人姓名、法人身份证号、法人手机号（可对接运营商三要素核验）。
- 结算账户信息：机构ID、开户名称、开户账号。
...
sequenceDiagram
    ...
    rect rgb(255, 240, 220)
        ...
        alt 法人信息验证
            ...
            U->>FE: 输入法人手机号并获取短信验证码
            U->>FE: 输入短信验证码
            FE->>AS: 运营商三要素核验（姓名+身份证+手机号）+ 短信验证码校验
        else 结算账户信息验证
            FE-->>U: 跳转结算账户信息验证页
...
| 法人信息验证 | 机构ID、法人姓名、法人身份证号、法人手机号、短信验证码 | 全部自由输入；身份证号 18 位、手机号 11 位、验证码 4 位；验证码需点击「获取验证码」后由法人手机号接收 |
| 结算账户信息验证 | 机构ID、开户名称、开户账号 | 全部自由输入；字符限制见下方 |
...
|---|---|---|
| 找回密码 | 绑定手机号、图片验证码、短信验证码、新密码 | — |
| 结算账户信息验证 | 机构ID、开户名称、开户账号 | 全部自由输入；字符限制同方案一 |
| 换绑手机号 | 新手机号、短信验证码 | 手机号 11 位；验证码 4 位 |
| 换绑成功 | 成功提示 + 继续找回密码按钮 | — |
...
| v1.2 | 2026-08-14 | 产品经理 | 法人信息验证增加运营商三要素核验（法人手机号）与手机短信验证码校验 |
| v1.3 | 2026-08-14 | 产品经理 | 结算账户信息验证去除开户行校验（仅保留机构ID、开户名称、开户账号） |
